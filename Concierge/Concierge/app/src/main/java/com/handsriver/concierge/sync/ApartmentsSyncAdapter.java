...
public class ApartmentsSyncAdapter extends AbstractThreadedSyncAdapter {

    public final String LOG_TAG = ApartmentsSyncAdapter.class.getSimpleName();
    public static final String APARTMENTS = "apartments";
    public static final String HTTP = "http";
    public static final String HTTPS = "https";
    private Context mContext;

    public ApartmentsSyncAdapter(Context context, boolean autoInitialize) {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        try {
            ...
            if (BASE_URL.length()>0 && (BASE_URL.startsWith(HTTP) || BASE_URL.startsWith(HTTPS))){
                Uri buildUri = Uri.parse(BASE_URL).buildUpon().appendPath(APARTMENTS).build();
                ...
                if (BASE_URL.startsWith(HTTPS)){
                    ...
                    apartmentsJsonStr = buffer.toString();
                    getApartmentsDataFromJson(apartmentsJsonStr);
    ...
    private void getApartmentsDataFromJson(String apartmentJsonStr)
            throws JSONException {

        final String APARTMENT_NUMBER = "apartment_number";
        final String ID_APARTMENT = "id_apartment";

        try {
            JSONArray apartmentsArray = new JSONArray(apartmentJsonStr);

            Vector<ContentValues> cVVector = new Vector<ContentValues>(apartmentsArray.length());
            ...
            for(int i = 0; i < apartmentsArray.length(); i++) {
                ...
                JSONObject apartmentJson = apartmentsArray.getJSONObject(i);

                apartment_id = apartmentJson.getLong(ID_APARTMENT);
                apartment_number = apartmentJson.getString(APARTMENT_NUMBER);

                ContentValues apartmentValues = new ContentValues();

                apartmentValues.put(ApartmentEntry.COLUMN_APARTMENT_NUMBER, apartment_number);
                apartmentValues.put(ApartmentEntry.COLUMN_APARTMENT_ID_SERVER, apartment_id);

                cVVector.add(apartmentValues);
            }

            IUApartments.run(cVVector);
