...
import com.handsriver.concierge.database.InsertUpdateTables.IUParceltype;
...
public class ParcelTypeSyncAdapter extends AbstractThreadedSyncAdapter {

    public final String LOG_TAG = ParcelTypeSyncAdapter.class.getSimpleName();
    public static final String PARCELTYPE = "parceltype";
    public static final String HTTP = "http";
    public static final String HTTPS = "https";
    private Context mContext;

    public ParcelTypeSyncAdapter(Context context, boolean autoInitialize) {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        try {
            ...
            if (BASE_URL.length()>0 && (BASE_URL.startsWith(HTTP) || BASE_URL.startsWith(HTTPS))){
                Uri buildUri = Uri.parse(BASE_URL).buildUpon().appendPath(PARCELTYPE).build();
                ...
                if (BASE_URL.startsWith(HTTPS)){
                    ...
                    parceltypesJsonStr = buffer.toString();
                    getParcelTypeDataFromJson(parceltypesJsonStr);
    ...
    private void getParcelTypeDataFromJson(String parceltypeJsonStr)
            throws JSONException {

        final String PARCELTYPE = "type";
        final String ID_PARCELTYPE = "id_parceltype";

        try {
            JSONArray parceltypeArray = new JSONArray(parceltypeJsonStr);

            Vector<ContentValues> cVVector = new Vector<ContentValues>(parceltypeArray.length());

            for(int i = 0; i < parceltypeArray.length(); i++) {

                long parceltype_id;
                String type;

                JSONObject parceltypeJson = parceltypeArray.getJSONObject(i);

                parceltype_id = parceltypeJson.getLong(ID_PARCELTYPE);
                type = parceltypeJson.getString(PARCELTYPE);

                ContentValues parceltypeValues = new ContentValues();

                parceltypeValues.put(ParcelTypeEntry.COLUMN_PARCELTYPE_TYPE, type);
                parceltypeValues.put(ParcelTypeEntry.COLUMN_PARCELTYPE_ID_SERVER, parceltype_id);

                cVVector.add(parceltypeValues);
            }

            IUParceltype.run(cVVector);
